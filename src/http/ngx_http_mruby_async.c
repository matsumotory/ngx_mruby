...
static ngx_int_t ngx_mrb_post_fiber(ngx_mrb_reentrant_t *re, ngx_http_mruby_ctx_t *ctx)
{
  ...
  if (re->fiber != NULL) {
    ...
    if (mrb_test(ngx_mrb_run_fiber(re->mrb, re->fiber, ctx->async_handler_result))) {
      ...
      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, rc->connection->log, 0, "ngx_mrb_post_fiber mrb_test true: %d", 0);
      return NGX_DONE;
    } else {
      // can not resume the fiber, the fiber was finished
      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, rc->connection->log, 0, "ngx_mrb_post_fiber mrb_test false: %d", 0);
