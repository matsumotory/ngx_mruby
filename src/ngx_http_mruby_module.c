...
static ngx_int_t ngx_mrb_run(ngx_http_request_t *r, ngx_mrb_state_t *state,
                             ngx_mrb_code_t *code, ngx_flag_t cached,
                             ngx_str_t *result);
...
static ngx_int_t ngx_http_mruby_state_reinit_from_file(ngx_mrb_state_t *state,
                                                       ngx_mrb_code_t *code);
static ngx_mrb_code_t *
ngx_http_mruby_mrb_code_from_file(ngx_pool_t *pool, ngx_str_t *code_file_path);
...
static ngx_int_t
ngx_http_mruby_server_rewrite_inline_handler(ngx_http_request_t *r);
...
static ngx_int_t ngx_http_mruby_body_filter_handler(ngx_http_request_t *r,
                                                    ngx_chain_t *in);
static ngx_int_t
ngx_http_mruby_body_filter_inline_handler(ngx_http_request_t *r,
                                          ngx_chain_t *in);

static ngx_command_t ngx_http_mruby_commands[] = {

    {ngx_string("mruby_init_code"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_http_mruby_init_inline, NGX_HTTP_MAIN_CONF_OFFSET, 0, NULL},

    {ngx_string("mruby_init"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE12,
     ngx_http_mruby_init_phase, NGX_HTTP_MAIN_CONF_OFFSET, 0, NULL},

    {ngx_string("mruby_init_worker_code"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_http_mruby_init_worker_inline, NGX_HTTP_MAIN_CONF_OFFSET, 0, NULL},

    {ngx_string("mruby_init_worker"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE12,
     ngx_http_mruby_init_worker_phase, NGX_HTTP_MAIN_CONF_OFFSET, 0, NULL},

    {ngx_string("mruby_exit_worker_code"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_http_mruby_exit_worker_inline, NGX_HTTP_MAIN_CONF_OFFSET, 0, NULL},

    {ngx_string("mruby_exit_worker"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE12,
     ngx_http_mruby_exit_worker_phase, NGX_HTTP_MAIN_CONF_OFFSET, 0, NULL},

    {ngx_string("mruby_cache"),
     NGX_HTTP_LOC_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_mruby_loc_conf_t, cached), NULL},

    {ngx_string("mruby_add_handler"),
     NGX_HTTP_LOC_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_mruby_loc_conf_t, add_handler), NULL},
    ...
    {ngx_string("mruby_set"),
     ...
     ngx_http_mruby_set, NGX_HTTP_LOC_CONF_OFFSET, 0,
     ngx_http_mruby_set_handler},

    {ngx_string("mruby_set_code"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_HTTP_LIF_CONF | NGX_CONF_2MORE,
     ngx_http_mruby_set_inline, NGX_HTTP_LOC_CONF_OFFSET, 0,
    ...
    {ngx_string("mruby_output_filter"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_HTTP_LIF_CONF | NGX_CONF_TAKE12,
     ngx_http_mruby_body_filter_phase, NGX_HTTP_LOC_CONF_OFFSET, 0,
     ngx_http_mruby_body_filter_handler},

    {ngx_string("mruby_output_filter_code"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_HTTP_LIF_CONF | NGX_CONF_TAKE1,
     ngx_http_mruby_body_filter_inline, NGX_HTTP_LOC_CONF_OFFSET, 0,
...
ngx_module_t ngx_http_mruby_module = {
    NGX_MODULE_V1, &ngx_http_mruby_module_ctx, /* module context */
...
static ngx_int_t ngx_http_mruby_init(ngx_conf_t *cf)
{
  ...
  ngx_conf_log_error(NGX_LOG_NOTICE, cf, 0, "%s/%s (%s/%s) mechanism enabled",
                     MODULE_NAME, MODULE_VERSION, MRUBY_RUBY_ENGINE,
                     MRUBY_VERSION);
                            ...
                            ngx_mrb_code_t *code)
{
  ...
  ngx_log_error(NGX_LOG_INFO, cycle->log, 0, "%s INFO %s:%d: mrb_run",
...
{
  int ai = mrb_gc_arena_save(state->mrb);
  ngx_log_error(NGX_LOG_INFO, cf->log, 0, "%s INFO %s:%d: mrb_run", MODULE_NAME,
                __func__, __LINE__);
  mrb_run(state->mrb, code->proc, mrb_top_self(state->mrb));
  if (state->mrb->exc) {
                      ...
                      ngx_str_t *result)
{
  ...
  if (ctx == NULL && (ctx = ngx_pcalloc(r->pool, sizeof(*ctx))) == NULL) {
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                  "failed to allocate memory from r->pool %s:%d", __FUNCTION__,
                  __LINE__);
    return NGX_ERROR;
  }
  ...
  if (!cached && !code->cache) {
    ...
    ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
                  "%s INFO %s:%d: mrb_run info: ai=%d", MODULE_NAME, __func__,
                  __LINE__, ai);
  }
  ...
  if (state->mrb->exc) {
    ...
    if (mrb_nil_p(mrb_result)) {
      ...
      ngx_memcpy(result->data,
                 (u_char *)mrb_str_to_cstr(state->mrb, mrb_result), result_len);
                                                     ...
                                                     ngx_mrb_code_t *code)
{
  ...
  if (code->code_type == NGX_MRB_CODE_TYPE_FILE) {
    ...
    ngx_conf_log_error(
        NGX_LOG_NOTICE, cf, 0, "%s NOTICE %s:%d: compile info: "
...
{
  ngx_http_mruby_main_conf_t *mmcf =
      ngx_http_conf_get_module_main_conf(cf, ngx_http_mruby_module);
  ngx_str_t *value;
  ngx_mrb_code_t *code;
  ngx_int_t rc;
                                      ...
                                      void *conf, code_type_t type)
{
  ...
  if (filter_data->code == NGX_CONF_UNSET_PTR) {
    if (type == NGX_MRB_CODE_TYPE_FILE) {
      ngx_conf_log_error(NGX_LOG_ERR, cf, 0,
                         ...
                         target.data, filter_data->script.data);
    }
    return NGX_CONF_ERROR;
  }
  ...
  ngx_conf_log_error(NGX_LOG_NOTICE, cf, 0,
                     "%s NOTICE %s:%d: target variable=(%s)", MODULE_NAME,
                     __FUNCTION__, __LINE__, target.data);
...
#define NGX_MRUBY_DEFINE_METHOD_NGX_HANDLER(handler_name, code)                \
  static ngx_int_t ngx_http_mruby_##handler_name##_handler(                    \
      ngx_http_request_t *r)                                                   \
  {                                                                            \
    ngx_http_mruby_main_conf_t *mmcf =                                         \
        ngx_http_get_module_main_conf(r, ngx_http_mruby_module);               \
    ngx_http_mruby_loc_conf_t *mlcf =                                          \
        ngx_http_get_module_loc_conf(r, ngx_http_mruby_module);                \
...
static ngx_int_t ngx_http_mruby_content_handler(ngx_http_request_t *r)
{
  ...
  if (mlcf->add_handler) {
    if (ngx_http_map_uri_to_path(r, &path, &root, 0) == NULL) {
      ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                    "%s:%d request_file(%s) map failed", __FUNCTION__, __LINE__,
                    path.data);
      return NGX_ERROR;
    }
    if (access((const char *)path.data, F_OK) != 0) {
      ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
                    "%s:%d request_file(%s) not found", __FUNCTION__, __LINE__,
                    path.data);
    ...
    if (code == NGX_CONF_UNSET_PTR) {
      ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                    "%s:%d mrb_file(%s) open failed", __FUNCTION__, __LINE__,
                    path.data);
      return NGX_ERROR;
    }
  } else {
  ...
  if (!code->cache) {
    NGX_MRUBY_STATE_REINIT_IF_NOT_CACHED(mlcf->cached, mmcf->state, code,
                                         ngx_http_mruby_state_reinit_from_file);
  }
  return ngx_mrb_run(r, mmcf->state, code, mlcf->cached, NULL);
}
                                                    ...
                                                    ngx_chain_t *in)
{
  ...
  if ((rc = ngx_http_mruby_read_body(r, in, ctx)) != NGX_OK) {
    if (rc == NGX_AGAIN) {
      ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                    "failed to read body. NGX_AGAIN %s:%d", __FUNCTION__,
                    __LINE__);
      return NGX_OK;
    }
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                  "failed to read body %s:%d", __FUNCTION__, __LINE__);
    return NGX_ERROR;
  }
  ...
  if (!mlcf->body_filter_code->cache) {
    NGX_MRUBY_STATE_REINIT_IF_NOT_CACHED(mlcf->cached, mmcf->state,
                                          ...
                                          ngx_chain_t *in)
{
  ...
  rc = ngx_mrb_run(r, mmcf->state, mlcf->body_filter_inline_code, mlcf->cached,
                   NULL);
  if (rc == NGX_ERROR) {
    return NGX_ERROR;
  }
